...
import com.example.eye.logic.model.Author
import com.example.eye.logic.model.Consumption
import com.example.eye.logic.model.Cover
import com.example.eye.logic.model.WebUrl
...
class NewDetailActivity : BaseActivity() {
    ...
    private val viewModel by lazy {
        ViewModelProvider(this,
            InjectorUtil.getNewDetailViewModelFactory()).get(NewDetailViewModel::class.java)
    }
    ...
    private fun initParams() {
        ...
        if (intent.getLongExtra(EXTRA_VIDEO_ID, 0L) != 0L) viewModel.videoId = intent.getLongExtra(
            EXTRA_VIDEO_ID, 0L)
    }
        ...
        if (!viewModel.videoDetailLiveData.hasObservers()) {
            viewModel.videoDetailLiveData.observe(this, Observer { result ->
                ...
                response.videoBeanForClient?.run {
                    viewModel.videoInfoData = VideoInfo(id,
                        playUrl,
                        title,
                        description,
                        category,
        ...
        override fun onClick(v: View) {
            viewModel.videoInfoData?.let {
                when (v) {
                    ...
                    ivCollection -> start<LoginActivity>(this@NewDetailActivity)
                    ...
                    ivAvatar, etComment -> start<LoginActivity>(this@NewDetailActivity)
